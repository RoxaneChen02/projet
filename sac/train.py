...
class PPOTrainer :
    ...
    def train(folder_path, model_name):
        env = CarRacing(lidar=True, verbose=1)
        model = PPO("MlpPolicy", env, learning_rate = 7.3e-4, buffer_size= 300000, batch_size= 256,gamma= 0.99, ent_coef= 'auto',  tau= 0.02, train_freq= 8, gradient_steps= 10, learning_starts= 1000, use_sde= True,use_sde_at_warmup= True,  verbose=1, tensorboard_log=folder_path+"/PPO_car_racing_lidar_tensorboard/")
        model.learn(total_timesteps=5e4, log_interval=1, tb_log_name="PPO_car_racing_lidar")
        model.save(folder_path+model_name+".zip")
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        eval_env  = CarRacing(lidar=True, verbose=1)
